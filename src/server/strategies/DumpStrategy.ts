...
export default class DumpStrategy extends BaseStrategy {
    ...
    private static readonly CONTAINER_TMP_DIR = '/zenith-backup-tmp'
    ...
    constructor(plan: Plan) {
        ...
        this.docker_volumes = config.docker_volumes
        this.docker_env = config.docker_env
    }
    ...
    private buildDockerArgs(extraVolumes: string[] = []): string[] {
        const args = ['run', '--rm', '-v', `${DumpStrategy.VOLUME}:/dumps`]
    ...
    public async backup(metadata: Record<string, any> = {}): Promise<DumpSnapshot> {
        ...
        const command = this.backup_command.replace('{output}', DumpStrategy.CONTAINER_TMP_DIR)
