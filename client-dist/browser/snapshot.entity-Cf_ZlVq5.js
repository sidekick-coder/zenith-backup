...
function M(t, e) {
  const n = O(), r = e?.weekStartsOn ?? e?.locale?.options?.weekStartsOn ?? n.weekStartsOn ?? n.locale?.options?.weekStartsOn ?? 0, a = f(t, e?.in), o = a.getDay(), i = (o < r ? 7 : 0) + o - r;
...
function B(t, e, n) {
  const [r, a] = _(
    ...
    t,
    e
  ), o = D(r), i = D(a), u = +o - v(o), h = +i - v(i);
  return Math.round((u - h) / 864e5);
}
function H(t, e) {
...
function X(t) {
  return !(!Q(t) && typeof t != "number" || isNaN(+f(t)));
}
function j(t, e) {
  const n = f(t, e?.in);
...
function p(t) {
  return (e, n = {}) => {
    ...
    if (!o)
      return null;
    const i = o[0], u = r && t.parsePatterns[r] || t.parsePatterns[t.defaultParseWidth], h = Array.isArray(u) ? ot(u, (c) => c.test(i)) : (
...
function Ot(t, e) {
  const n = f(t, e?.in), r = +P(n) - +H(n);
  return Math.round(r / 6048e5) + 1;
}
function C(t, e) {
  const n = f(t, e?.in), r = n.getFullYear(), a = O(), o = e?.firstWeekContainsDate ?? e?.locale?.options?.firstWeekContainsDate ?? a.firstWeekContainsDate ?? a.locale?.options?.firstWeekContainsDate ?? 1, i = g(e?.in || t, 0);
  i.setFullYear(r + 1, 0, o), i.setHours(0, 0, 0, 0);
  const u = M(i, e), h = g(e?.in || t, 0);
...
function xt(t, e) {
  const n = f(t, e?.in), r = +M(n, e) - +kt(n, e);
  return Math.round(r / 6048e5) + 1;
}
function s(t, e) {
...
const qt = /[yYQqMLwIdDecihHKkms]o|(\w)\1*|''|'(''|[^'])+('|$)|./g, Nt = /P+p+|P+|p+|''|'(''|[^'])+('|$)|./g, _t = /^'([^]*?)'?$/, Bt = /''/g, Ht = /[a-zA-Z]/;
function Xt(t, e, n) {
  const r = O(), a = r.locale ?? Mt, o = r.firstWeekContainsDate ?? r.locale?.options?.firstWeekContainsDate ?? 1, i = r.weekStartsOn ?? r.locale?.options?.weekStartsOn ?? 0, u = f(t, n?.in);
  if (!X(u))
...
class jt {
  ...
  get humanSize() {
    if (!this.size)
      return "0 B";
    const e = Math.floor(Math.log(this.size) / Math.log(1024)), n = ["B", "KB", "MB", "GB", "TB", "PB", "EB", "ZB", "YB"];
    return (this.size / Math.pow(1024, e)).toFixed(2) + " " + n[e];
  }
}
export {
  jt as S,
